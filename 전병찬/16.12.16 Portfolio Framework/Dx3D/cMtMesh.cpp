...
cMtMesh::cMtMesh(char* szFolder, char* szFilename)
	: m_pRootFrame(NULL)
	...
	, m_pEffect(NULL)
{
	cMtMesh* pSkinnedMesh = g_pMtMeshManager->GetSkinnedMesh(szFolder, szFilename);
...
void cMtMesh::Load(char* szDirectory, char* szFilename)
{
...
void cMtMesh::UpdateAndRender(D3DXMATRIXA16* pmat)
{
...
void cMtMesh::Render(ST_BONE* pBone /*= NULL*/)
{
	assert(pBone);
...
LPD3DXEFFECT cMtMesh::LoadEffect(char* szFilename)
{
...
void cMtMesh::Update(ST_BONE* pCurrent, D3DXMATRIXA16* pmatParent)
{
...
void cMtMesh::SetAnimationIndex(int nIndex)
{
	SettingBlend();
	if (!m_pAnimController)
		return;
...
void cMtMesh::SettingBlend()
{
	LPD3DXANIMATIONSET pAniSet1 = NULL;
	m_pAnimController->ResetTime();
	m_pAnimController->SetTrackPosition(0, 0);
	m_pAnimController->SetTrackEnable(1, true);
	m_pAnimController->GetTrackAnimationSet(0, &pAniSet1);
	m_pAnimController->SetTrackAnimationSet(1, pAniSet1);

	Tweight0 = 0;
	Tweight1 = 1;
	SAFE_RELEASE(pAniSet1);
}

void cMtMesh::UpdateAnimation()
{
	if (Tweight0 < 1)
	{
		Tweight0 += 0.05f;
		m_pAnimController->SetTrackWeight(0, Tweight0);

		Tweight1 -= 0.05f;
		m_pAnimController->SetTrackWeight(1, Tweight1);
	}
	else
	{
		Tweight0 = 1;
...
void cMtMesh::SetRandomTrackPosition()
{
