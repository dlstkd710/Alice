...
void cM_Nightmare::Update(D3DXVECTOR3 _char_position, iMap* m_pMap)
{
	...
	{
		m_Mesh->SetAnimationIndex(3);
	}
	if (g_pkeyManager->isOnceKeyDown('V'))
	{
		m_Mesh->SetAnimationIndex(4);
	}
	if (g_pkeyManager->isOnceKeyDown('B'))
	{
		m_Mesh->SetAnimationIndex(5);
	}
...
void cM_Nightmare::MoveMt()
{
	if (ActionCheck)
	{
		D3DXMatrixRotationY(&m_matWorld, D3DX_PI / 4.0f);
		...
		_cActionMove->SetpCheck(ActionCheck);
