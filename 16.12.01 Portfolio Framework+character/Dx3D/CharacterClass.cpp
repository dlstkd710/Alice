...
{
	_Animation = new cAnimation("./Alice45/", "AliceW.x");
	
	_Knife = new cAnimation("./Knife/", "Kife.X");

	_Animation->SetAnimationIndex(9);
	
	_Knife->SetPosition(D3DXVECTOR3(0, 0, 0));
	_Animation->SetPosition(D3DXVECTOR3(5, 0, 5));
}

void CharacterClass::SetupMom(cAnimation * ANI, 
	char * foderName,
	char * fileName, 
	int intdex, 
	D3DXVECTOR3 pos)
{
	ANI = new cAnimation(foderName, fileName);
	ANI->SetAnimationIndex(intdex);
	ANI->SetPosition(pos);
}
...
void CharacterClass::Render()
{
	...
	// 전투이시에 만 동작
	if (m_eState == E_STATE_IDEL || m_eState == E_STATE_RUNIDEL || m_eState == E_STATE_1COMBO || m_eState == E_STATE_2COMBO || m_eState == E_STATE_3COMBO) {
...
void CharacterClass::SetintdexAnimationLep(int intdex)
{
	...
	pAC->KeyTrackEnable(m_dwCurrentTrack, FALSE, g_pTimeManager->GetDeltaTime() + b);
	pAC->KeyTrackSpeed(m_dwCurrentTrack, 0.0f, g_pTimeManager->GetDeltaTime(), b / 3, D3DXTRANSITION_LINEAR);
	pAC->KeyTrackWeight(m_dwCurrentTrack, 0.0f, g_pTimeManager->GetDeltaTime(),  b / 3, D3DXTRANSITION_LINEAR);
	
	pAC->SetTrackEnable(dwNewTrack, TRUE);
	pAC->KeyTrackSpeed(dwNewTrack, 1.0f, g_pTimeManager->GetDeltaTime(),  b / 3, D3DXTRANSITION_LINEAR);
	pAC->KeyTrackWeight(dwNewTrack, 1.0f, g_pTimeManager->GetDeltaTime(),  b / 3, D3DXTRANSITION_LINEAR);
...
void CharacterClass::UpdateNormal(iMap* pMap /*= NULL*/)
{
	...
	else
	{
		...
		if (GetKeyState('W') & 0x8000) {
			_Animation->SetAnimationIndex(5);
