...
void cM_Fly::Update(D3DXVECTOR3 _char_position, iMap* m_pMap)
{
	...
	if (m_pMap)
	{
		...
		{
			if (VecTemp.y + 15 > m_vPosition.y);
			{
				m_vPosition.y = VecTemp.y + 15;
			}
	...
	case ATTACKREADY:
		...
		{
			_countAttack++;
			if (_countAttack % 10 == 0)
			{
				m_NowGo = m_CharacterPos;
				m_Stastus = ATTACK_FLY;
				m_Mesh->SetAnimationIndex(1);
				_countAttack = 0;
				MoveMt();
			}
...
void cM_Fly::MoveMt()
{
	...
	case ATTACK_FORWARD:
		...
		break;
	case ATTACK_FLY:
		_cActionMove->SetActionTime(t*0.02f);
		break;
	}
...
{
	D3DXVECTOR3 _CheckobbPos = m_CharacterPos;
	_CheckobbPos.y = m_CharacterPos.y - 15;
	cOBB* Player_obb;
	Player_obb = new cOBB;
	Player_obb->Setup(D3DXVECTOR3(_CheckobbPos.x - 1.0f, _CheckobbPos.y + 5, _CheckobbPos.z - 1.0f),
		D3DXVECTOR3(_CheckobbPos.x + 1.0f, _CheckobbPos.y + 15, _CheckobbPos.z + 1.0f));
