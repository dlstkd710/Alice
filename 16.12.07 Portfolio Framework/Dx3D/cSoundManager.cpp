...
BOOL cSoundManager::CreateDirectSound(HWND hWnd)
{
	//다이렉트 사운드 개체 생성
...
BOOL cSoundManager::LoadWave(LPSTR lpFileName, LPDIRECTSOUNDBUFFER * lpDSBuffer)
{
...
BOOL cSoundManager::SetVolume(LPDIRECTSOUNDBUFFER lpDSBuffer, LONG lVolume)
{
...
BOOL cSoundManager::SetPan(LPDIRECTSOUNDBUFFER lpDSBuffer, LONG lPan)
{
...
void cSoundManager::DeleteDirectSound()
{
	g_lpDS->Release();
	g_lpDS = NULL;
}

void cSoundManager::Play(LPDIRECTSOUNDBUFFER lpDSBuffer, BOOL Loop)
{
	//버퍼가 비어있으면 종료
...
void cSoundManager::stop(LPDIRECTSOUNDBUFFER lpDSBuffer)
{
	if (lpDSBuffer == NULL) return;
	lpDSBuffer->Stop();
	g_bPlay = FALSE;
