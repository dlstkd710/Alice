...
LPDIRECT3DTEXTURE9 cTextureManager::GetTexture(char * szPath, D3DXIMAGE_INFO *m_stImageInfo)
{
	return GetTexture(std::string(szPath),m_stImageInfo);
}

LPDIRECT3DTEXTURE9 cTextureManager::GetTexture(std::string sPath, D3DXIMAGE_INFO *m_stImageInfo)
{
	if (m_mapTexture.find(sPath) == m_mapTexture.end())
	{
		LPDIRECT3DTEXTURE9 pTexture;
